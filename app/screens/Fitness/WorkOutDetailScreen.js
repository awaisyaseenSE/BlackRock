...
export default function WorkOutDetailScreen({route}) {
  ...
  const getBodyPartDatail = async () => {
    setLoading(true);
    const url = `https://exercisedb.p.rapidapi.com/exercises/bodyPart/${data?.name}?limit=10`;
    const options = {
      method: 'GET',
      headers: {
  ...
  return (
    <>
      <View style={styles.conatiner}>
        ...
        <FastImage source={{uri: data?.image}} style={styles.imageStyle} />
        <View
          style={[
            styles.topContainer,
            {
